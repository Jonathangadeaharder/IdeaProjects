...
from .config import PROJECT_DIR, PID_FILE, BACKEND_DIR, FRONTEND_DIR, ServerStatus, LOG_DIR
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / f"server_manager_{datetime.now().strftime('%Y%m%d')}.log"),
...
class ProfessionalServerManager:
                ...
                for name, info in self.state.get("servers", {}).items():
                    if name in self.servers and "pid" in info:
                        ...
                        if ProcessController.is_process_running(pid):
                            ...
                            logger.info(f"Previous {name} server (PID {pid}) is not running")
    ...
    def start_server(self, name: str) -> bool:
        ...
        try:
            ...
            max_wait_time = 60 if name == "backend" else 30  # Backend needs more time for Whisper model
            poll_interval = 2
            elapsed = 0
            ...
            while elapsed < max_wait_time:
                ...
                if server.check_health():
                    ...
                    logger.info(f"[SUCCESS] {name} server started successfully on port {server.port} after {elapsed}s")
                    self._save_state()
                    return True
                
                # Show progress with different messages based on timing
                if elapsed == 0:
                    logger.info(f"  Process started with PID {server.pid}, waiting for initialization...")
                elif elapsed == 10:
                    logger.info(f"  {name} server is initializing... (this is normal)")
                elif elapsed == 20 and name == "backend":
                    logger.info(f"  Backend is loading AI models... (this may take a moment)")
                elif elapsed == 30 and name == "backend":
                    logger.info(f"  Still loading Whisper model... (large model takes time)")
                elif elapsed > 0 and elapsed % 10 == 0:
                    logger.info(f"  Still waiting for {name} server... ({elapsed}s elapsed)")
