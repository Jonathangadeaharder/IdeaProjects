...
function formatZodIssue(issue: ZodIssue): string {
  ...
  switch (code) {
    case 'too_small':
      if ('minimum' in issue) {
        const tooSmallIssue = issue as { minimum: number }
        return `Must be at least ${tooSmallIssue.minimum} characters`
      }
      return message

    case 'too_big':
      if ('maximum' in issue) {
        const tooBigIssue = issue as { maximum: number }
        return `Must be at most ${tooBigIssue.maximum} characters`
      }
      return message

    case 'invalid_string':
      if ('validation' in issue) {
        const stringIssue = issue as { validation: string }
