...
async def test_Whenlist_videosCalled_ThenReturnsempty_when_path_missing(
    async_client, url_builder, monkeypatch, tmp_path
):
...
@pytest.mark.asyncio
@pytest.mark.timeout(30)
async def test_WhenGetSubtitlesServesExistingFile_ThenSucceeds(async_client, url_builder, monkeypatch, tmp_path):
    ...
    response = await async_client.get(
        url_builder.url_for("get_subtitles", subtitle_path="example.srt"), headers=headers
    )
...
@pytest.mark.asyncio
@pytest.mark.timeout(30)
async def test_WhenStreamVideoSetsAcceptRanges_ThenSucceeds(async_client, url_builder, monkeypatch, tmp_path):
    ...
    response = await async_client.get(
        url_builder.url_for("stream_video", series="Series", episode="Episode%201.mp4"), headers=headers
    )
