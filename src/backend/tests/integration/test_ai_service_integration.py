...
class TestTranscriptionServiceIntegration:
    ...
    def test_whisper_tiny_actual_transcription(self):
        ...
        try:
            ...
            # Result is a TranscriptionResult object with full_text attribute
            assert hasattr(result, "full_text") or hasattr(result, "text") or isinstance(result, dict)
            if isinstance(result, dict):
                assert "text" in result or "full_text" in result
                text = result.get("text") or result.get("full_text")
                assert "Hallo Welt" in text
            elif hasattr(result, "full_text"):
                assert "Hallo Welt" in result.full_text
            else:
