...
class VocabularyRepository(BaseRepository[VocabularyWordModel], VocabularyRepositoryInterface):  # type: ignore[misc]
    ...
    def _to_domain_entity(self, model: VocabularyWordModel) -> VocabularyWord:
        ...
        # Fail fast on invalid difficulty level - database should have valid data
        difficulty_level = DifficultyLevel(model.level or "A1")

        # word_type may legitimately be unknown for unclassified words
        word_type_value = getattr(model, "word_type", None) or "unknown"
        word_type = WordType(word_type_value)
