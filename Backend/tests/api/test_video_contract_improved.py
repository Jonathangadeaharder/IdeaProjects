...
async def test_WhenGetVideosIncludesExpectedFields_ThenSucceeds(async_client, url_builder):
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_Whenstream_videoCalled_ThenReturnscontent_when_exists(async_client, url_builder):
    ...
    try:
        ...
        with patch("services.videoservice.video_service.VideoService.get_video_file_path") as mock_path:
            ...
            response = await async_client.get(
                url_builder.url_for("stream_video", series="series", episode="S01E01"), headers=headers
            )
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_Whensubtitle_download_MissingFileCalled_ThenSucceeds(async_client, url_builder):
    """Invalid input: requesting missing subtitle returns 404."""
    headers = await _auth(async_client)

    with patch("os.path.exists", return_value=False):
        response = await async_client.get(
            url_builder.url_for("get_subtitles", subtitle_path="missing.srt"), headers=headers
        )
...
@pytest.mark.anyio
@pytest.mark.timeout(30)
async def test_Whenvideo_uploadWithnon_video_ThenRejects(async_client, url_builder):
