...
class TestProcessSrtContent:
    ...
    def test_process_srt_content_highlights_words(self, service):
        """Test vocabulary word highlighting in SRT"""
        # Arrange
        srt_content = """1
00:00:00,000 --> 00:00:02,000
Das Haus ist groß
...
Der Hund ist klein
"""
        vocab_words = {"haus", "hund"}

        # Act
        result = service._process_srt_content(srt_content, vocab_words)

        # Assert - check that processing completed
        assert "00:00:00,000 --> 00:00:02,000" in result
        assert "00:00:02,000 --> 00:00:04,000" in result

    def test_process_srt_content_preserves_structure(self, service):
        """Test that SRT structure is preserved"""
        # Arrange
...
class TestHighlightVocabularyInLine:
    """Test vocabulary highlighting in subtitle lines"""

    @pytest.fixture
    def service(self):
        return ChunkProcessingService(AsyncMock())

    def test_highlight_vocabulary_case_insensitive(self, service):
        """Test highlighting works case-insensitively"""
        # Arrange
        line = "Das Haus ist sehr groß"
        vocab_words = {"haus"}

        # Act
        result = service._highlight_vocabulary_in_line(line, vocab_words)

        # Assert - check highlighting applied
        assert "Haus" in result or "haus" in result

    def test_highlight_vocabulary_whole_words_only(self, service):
        """Test highlighting only matches whole words"""
        # Arrange
        line = "Hausaufgaben sind wichtig"
        vocab_words = {"haus"}

        # Act
        result = service._highlight_vocabulary_in_line(line, vocab_words)

        # Assert - "haus" in "Hausaufgaben" should not be highlighted as separate word
        assert "Hausaufgaben" in result

    def test_highlight_multiple_words(self, service):
        """Test highlighting multiple vocabulary words"""
        # Arrange
        line = "Der Hund und die Katze"
        vocab_words = {"hund", "katze"}

        # Act
        result = service._highlight_vocabulary_in_line(line, vocab_words)

        # Assert
        assert "Hund" in result or "hund" in result
        assert "Katze" in result or "katze" in result


class TestApplySelectiveTranslations:
    """Test selective translation application"""
    ...
    @pytest.mark.anyio
    async def test_apply_selective_translations_error_handling(self, service):
        """Test error handling in selective translations"""
        # Arrange
        with patch('services.filtering.translation_analyzer.TranslationAnalyzer') as MockAnalyzer:
            mock_analyzer = MockAnalyzer.return_value
            mock_analyzer.refilter_for_translations = AsyncMock(side_effect=Exception("Translation error"))

            # Act
            result = await service.apply_selective_translations(
                srt_path="/test.srt",
                known_words=["Hallo", "Welt"],
                target_language="de",
                user_level="A1",
                user_id="user123"
            )

            # Assert - returns fallback response
