...
class TestHealthMonitor:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.critical_services = [
            ...
            'LoggingService', 'DirectSubtitleProcessor'
        ]
    ...
    def run_tests_with_monitoring(self) -> TestRunMetrics:
        ...
        try:
            ...
            metrics = self._calculate_test_metrics(
                test_results, total_duration, pollution_detected, isolation_issues
            )
    ...
    def _calculate_test_metrics(self, test_results: List[TestResult],
                              duration: float, pollution: bool,
                              isolation_issues: List[str]) -> TestRunMetrics:
    ...
    def analyze_trends(self, lookback_days: int = 7) -> List[HealthTrend]:
        ...
        success_trend = self._calculate_trend(
            "success_rate",
            latest_metrics.get("success_rate", 0),
        ...
        duration_trend = self._calculate_trend(
            "test_duration",
            latest_metrics.get("total_duration", 0),
        ...
        failed_trend = self._calculate_trend(
            "failed_tests",
            latest_metrics.get("failed", 0),
            previous_metrics.get("failed", 0)
        )
        trends.append(failed_trend)

        return trends

    def _load_metrics(self, file_path: Path) -> Optional[Dict]:
