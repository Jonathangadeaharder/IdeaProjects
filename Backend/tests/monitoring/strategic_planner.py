...
class StrategicCoveragePlanner:
    ...
    def _extract_service_name(self, file_path: Path) -> Optional[str]:
    ...
    def _analyze_single_service(self, file_path: Path, service_name: str,
                               coverage_data: Dict[str, float]) -> ServiceAnalysis:
    ...
    def _find_dependencies(self, content: str, tree: Optional[ast.AST]) -> List[ServiceDependency]:
    ...
    def _find_public_methods(self, content: str, tree: Optional[ast.AST]) -> List[str]:
    ...
    def _calculate_complexity_score(self, content: str, tree: Optional[ast.AST],
                                  public_methods: List[str]) -> float:
    ...
    def _load_current_coverage(self) -> Optional[Dict]:
    ...
    def _create_service_plan(self, analysis: ServiceAnalysis) -> CoveragePlan:
        ...
        if analysis.business_criticality == 'high' and analysis.current_coverage < 50:
            priority = 'critical'
        elif analysis.business_criticality == 'high' or (analysis.business_criticality == 'medium' and analysis.current_coverage < 30):
            priority = 'high'
    ...
    def _estimate_timeline(self, plans: List[CoveragePlan]) -> str:
        ...
        critical_high = len([p for p in plans if p.priority in ['critical', 'high']])
