...
class RefinedGermanBaseFormAnalyzer:
    def __init__(self):
        ...
        self.correct_base_forms = {
            ...
            'arbeiten', 'sprechen', 'verstehen', 'antworten', 'bedeuten', 'begleiten',
            'behalten', 'behaupten', 'beobachten', 'beraten', 'berichten', 'anbieten',
            'aufhalten', 'auftreten', 'ausrichten', 'beachten', 'beantworten', 'achten',

            # Nouns that end in -er but are correct base forms
            'Vater', 'Mutter', 'Lehrer', 'Fahrer', 'Verkäufer', 'Arbeiter', 'Computer',
            'Hamburger', 'Finger', 'Winter', 'Sommer', 'Bruder', 'Schwester', 'Besucher',
            'Meter', 'Liter', 'Kilometer', 'Theater', 'Wetter', 'Vermieter', 'Anbieter',
            'Empfänger', 'Absender', 'Sprecher', 'Hörer', 'aber', 'ander', 'unter',
            'über', 'auseinander', 'durcheinander', 'gegenüber', 'besonder', 'bitter',
            'bisher', 'entweder',
            ...
            'Karte', 'Torte', 'Sahne', 'Schere', 'Garage', 'Orange', 'Minute', 'Bitte',
            'beide', 'beinahe', 'danke', 'gerade',

            # Other correct base forms
            'Best', 'Test', 'Rest', 'West', 'Ost', 'Nord', 'Süd', 'Text', 'Fest',
            'Ernst', 'August', 'Herbst', 'Frost', 'Durst', 'Verlust', 'Kunst',
            'absolut', 'bereit', 'beliebt', 'berühmt', 'bestimmt', 'breit', 'direkt',
            'ernst', 'fest', 'damit', 'dicht', 'erschöpft', 'feucht', 'geeignet',
            'gesamt', 'befreit', 'begeistert', 'behindert', 'ausgebildet', 'ausgezeichnet',

            # Time and other expressions that are correct
            'früh', 'früher', 'jetzt', 'immer', 'nie', 'oft', 'manchmal', 'bald',
        }
    ...
    def generate_report(self, all_violations: Dict[str, List[Dict]]) -> str:
        ...
        report.append(f"SUMMARY: Found {total_violations} definite base form violations across {len(all_violations)} files")
...
def main():
    ...
    print("Running refined German vocabulary base form analysis...")
    print("(Focusing on high-confidence violations only)")
    print("=" * 60)
    ...
    for filename in csv_files:
        ...
        print(f"  Found {len(violations)} definite violations")

    print("=" * 60)
    print(f"Analysis complete. Total definite violations found: {sum(len(v) for v in all_violations.values())}")
    ...
    print("\nDetailed report saved to: refined_base_form_violations_report.txt")
