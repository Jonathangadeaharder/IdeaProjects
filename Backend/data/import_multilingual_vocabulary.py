"""
Import German-Spanish vocabulary pairs from CSV files into the new multilingual database schema
"""
...
async def import_vocabulary_pairs(pairs: List[Tuple[str, str, str]], session: AsyncSession) -> int:
    """Import vocabulary pairs into the database"""
    imported_count = 0
    skipped_count = 0
            ...
            if existing_translation:
                ...
                if existing_spanish:
                    ...
                    spanish_translation = VocabularyTranslation(
                        ...
                        language_code="es",
                        word=spanish,
                        lemma=spanish  # Use the word as lemma for now
                    )
                ...
                concept = VocabularyConcept(
                    ...
                    semantic_category="unknown",  # Will be enhanced later
                    domain="general"
                )
                session.add(concept)
                ...
                german_translation = VocabularyTranslation(
                    ...
                    language_code="de",
                    word=german,
                    lemma=german  # Use the word as lemma for now
                )
    ...
    try:
        ...
        logger.error(f"Failed to commit vocabulary import: {e}")
        raise
    ...
    async with AsyncSessionLocal() as session:
        ...
        for csv_file in csv_files:
            ...
            if csv_path.exists():
                ...
                if pairs:
                    imported = await import_vocabulary_pairs(pairs, session)
                    total_imported += imported
                    logger.info(f"Imported {imported} pairs from {csv_file}")
                else:
