{
  ...
  "best_practices": {
    ...
    "business_assertions": "Exercise full user workflows (authenticate, perform the action, verify persisted state or API response) and assert on domain-specific outcomes, not just navigation success or element existence",
    "dynamic_url_detection": "Never hard-code localhost URLs - use port detection",
    "shared_config": "Resolve all environment details through the canonical helpers for the project (e.g., getFrontendUrl, backend fixtures); never duplicate URLs, credentials, or ports inside tests or helpers",
    "browser_cleanup": "Always close pages and browsers in afterEach/afterAll hooks",
    "deterministic_waits": "Replace setTimeout sleeps with explicit waits on selectors, API responses, or other observable events so failures surface quickly",
    "data_lifecycle": "Seed and clean data through dedicated fixtures/helpers so tests stay isolated and idempotent; ad-hoc CLI seed scripts or manual cleanup is not acceptable for automated suites",
    "unified_harnesses": "Prefer real test runners (Jest, Playwright, pytest) or the repository's orchestrator over bespoke node script.ts flows so failures are reported and integrated with CI",
    "shared_tooling": "Reuse the project's official helpers (e.g., orchestrators, test data managers, contract validators) instead of re-implementing setup/teardown logic in each suite"
  },
  "requirements": [
